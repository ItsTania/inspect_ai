...
| `INSPECT_LOG_DISPLAY_PY_LOGGER` | Name of the Python logger used by the `log` display. Defaults to the root logger. Set this to direct log display output to a named logger (e.g. for use with custom handlers). |
| `INSPECT_DISPLAY_SECONDARY` | Secondary display type to run alongside the primary display. Values: `full`, `rich`, `plain`, `log`, `none`. When set, both displays receive all updates. By default none, where no secondary display is set. |
