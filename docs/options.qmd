...
| `--log-display-py-logger` / `INSPECT_LOG_DISPLAY_PY_LOGGER` | Name of the Python logger used by the `log` display. Defaults to the root logger. Set this to direct log display output to a named logger (e.g. for use with custom handlers). |
| `--display-secondary` / `INSPECT_DISPLAY_SECONDARY` | Secondary display to run alongside the primary display. Only `log` is currently supported. When set, both displays receive all updates. |
