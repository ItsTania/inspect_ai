...
class _CompositeTaskScreen(TaskScreen):
    ...
    def input_screen(
        ...
        # Only the primary screen provides interactive terminal input
...
class CompositeDisplay(Display):
    ...
    def run_task_app(self, main: Callable[[], Coroutine[None, None, TR]]) -> TR:
        # This is invoked once per eval run and starts the event loop / app.
        # Only the primary display needs to do this, the secondary display receives updates passively through the other delegated methods.
        # Only one app can hold the TTY and one runtime can await main - we defer to the primary.
        return self.primary.run_task_app(main)

    @contextlib.contextmanager
    def suspend_task_app(self) -> Iterator[None]:
        # Pauses live terminal rendering around interactive prompts - relevant for the textual display (rich/plain/log are no-ops).
        # Only the primary display owns the terminal / event loop, so only it needs to
        # suspend for interactive input.
